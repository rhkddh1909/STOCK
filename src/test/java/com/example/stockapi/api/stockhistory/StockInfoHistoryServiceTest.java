...
class StockInfoHistoryServiceTest {
    ...
    public void stockInfoHistoryAnalysis_ThenCallfindStockInfoHistoryGroupData(){
        ...
        given(mockStockInfoHistroryRepository.findStockInfoHistoryGroupData(anyString(),anyString(),anyLong())).willReturn(Optional.of(List.of(stockInfoHistoryDto1)));

        //stockInfoHistories메소드를 서비스에서 호출 할 때 (when)
        List<StockInfoHistoryDto> stockInfoHistoryDtos = mockStockInfoHistoryService.stockInfoHistoryAnalysis("KOR","000660",1L);

        //findByNationAndStockCode 메서드가 호출되는지 검증
        then(mockStockInfoHistroryRepository).should().findStockInfoHistoryGroupData("KOR","000660",1L);
